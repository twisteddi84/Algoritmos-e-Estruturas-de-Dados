...
{
  hash_table_t **temporary_heads;
  int x;
  temporary_heads = (hash_table_node_t *)malloc(sizeof(hash_table_node_t) * hash_table->hash_table_size * 2);
  hash_table->hash_table_size *= 2;
  for(int i = 0;i < hash_table->hash_table_size;i++){
    x = crc32(hash_table->heads[i]) % hash_table->hash_table_size;
    temporary_heads[x] =  hash_table->heads[i];
  }
  free(hash_table->heads);
  hash_table->heads = temporary_heads;


  //iterar depois de criar o novo array
  //inserir à cabeça
...
{
  //allocate a temporary hash table

  //iterar sobre o array e depois percorrer cada uma das linked lists, e libertar tudo


  hash_table_t **temporary_heads;
  temporary_heads = (hash_table_node_t **)malloc(sizeof(hash_table_node_t *) * hash_table->hash_table_size);
  for(int i = 0;i < hash_table->hash_table_size;i++){
    free(temporary_heads[i]);
  } 
  hash_table->heads = temporary_heads;
  free(hash_table);
}
...
static hash_table_node_t *find_word(hash_table_t *hash_table,const char *word,int insert_if_not_found)
{
  ...
  //colocar o grow fora da find word e aumentar sempre que for usada 50%


  //if insert_if_not_found == 1, insert the word if it is not found
  ...
  // insert the word if it is not found
  else{
      i = crc32(word) % hash_table->hash_table_size;
      for(node = hash_table->heads[i];node != NULL;node = node->next){
      if(strcmp(node->word,word) == 0){
      ...
      //criar novo node, alocar memoria para o novo node, depois colocar o node como a primeira posição e colocar a antiga heads como valor seguinte
      //criar um novo node fora da lista, indicar o ponteiro dele para heads e indicar o antigo heads para o seu next
      node = hash_table->heads[i];
      hash_table->heads[i] = allocate_hash_table_node();
      strcpy(hash_table->heads[i]->word,word);
      hash_table->heads[i]->next = node;
      hash_table->number_of_entries++;
      return hash_table->heads[i];
  }
  }
