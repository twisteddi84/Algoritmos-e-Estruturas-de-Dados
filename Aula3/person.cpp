...
class person
{
  private:
    ...
    int d_telefone = 0;
  public:
    person(const char *name = "",const char *address = "",const int telefone = 0); // constructor
    ~person(void);                                          // destructor
    void set_name(const char *name);
    void set_address(const char *address);
    void set_telefone(const int telefone);
    const int get_telefone(void);
...
void person::set_telefone(const int telefone)
{
    if(telefone>999999999 || telefone<100000000){
...
person::person(const char *name,const char *address, const int telefone)
{ // constructor
  if(debug != 0)
...
inline const int person::get_telefone(void)
{
  return d_telefone;
}
...
int main(void)
{
  ...
  person q("no name","no address",925964353);           // create a new object (no copy needed)

  cout << "Before change of address: " << p.get_name() << ", " << p.get_address() << ", " << p.get_telefone() << endl;
  p.set_address("bench 24, Central Park, New York");
  cout << "After change of address:  " << p.get_name() << ", " << p.get_address() << ", " << p.get_telefone() << endl;
